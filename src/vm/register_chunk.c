...
void register_chunk_disassemble(const RegisterChunk* chunk, bool include_debug) {
    ...
    printf("=== Register Chunk Disassembly ===\n");
    printf("Module: %s\n", (chunk->module && chunk->module->name) ? chunk->module->name : "<unnamed>");
    printf("Instructions: %u\n", chunk->code_count);
    printf("Constants: %u\n", chunk->constant_count);
    printf("Functions: %u\n", chunk->function_count);
    printf("Globals: %u\n", chunk->global_count);
    
    if (include_debug && chunk->debug && chunk->debug->source_file_count > 0) {
        printf("Source files: %u\n", chunk->debug->source_file_count);
    }
    
    printf("\n--- Instructions ---\n");
    for (uint32_t i = 0; i < chunk->code_count; i++) {
        uint32_t instruction = chunk->code[i];
        ...
        printf("%04X: %08X  OP_%02X R%d R%d R%d IMM=%d\n", 
               i, instruction, opcode, dst, src1, src2, imm);
    }
    ...
    if (chunk->function_count > 0) {
        printf("\n--- Functions ---\n");
        for (uint16_t i = 0; i < chunk->function_count; i++) {
            FunctionInfo* func = &chunk->functions[i];
            printf("F%d: %s @ %04X-%04X (params: %d)\n", 
                   i, func->name ? func->name : "<anonymous>", 
                   func->start_address, func->end_address, func->parameter_count);
        }
    }
